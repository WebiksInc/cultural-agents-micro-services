...
#         if _first_run:
#             logger.info("First run: Initializing group metadata and fetching initial messages from supervisornode")
...
#     logger.info(f"Supervisor cycle complete. Recent messages: {len(updated_state.get('recent_messages', []))} from supervisornode")
...
#     logger.info("Starting supervisor continuous loop from supervisornode")
...
#                 logger.info(f"Loaded persona for {agent_type}: {persona_data.get('first_name', 'Unknown')} from supervisornode")
#         except Exception as e:
