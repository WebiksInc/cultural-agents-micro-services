...
def save_personality_analysis(
    chat_id: str,
    user_id: str,
    big5_values: Optional[Dict[str, float]] = None,
    verbose: bool = True
) -> Dict[str, Any]:
    """
    ...
    Args:
        chat_id: Telegram chat ID
        user_id: User ID
        big5_values: Dict with big5 traits (openness, conscientiousness, etc.)
    ...
    # Get message count for confidence calculation
    messages = get_participant_messages(chat_id, user_id)
    message_count = len(messages)
    
    # Generate or use provided Big5 values
    if big5_values is None:
    ...
    # Calculate confidence based on message count and existing snapshots
    existing_snapshots = participant_data.get("personality_snapshots", [])
    base_confidence = min(0.3 + (message_count / 100) * 0.5, 0.9)
